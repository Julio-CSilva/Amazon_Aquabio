...
import {
  GRID_LINE, INK_MUTED, INK_SECONDARY, SURFACE, TRILHA, eixo, layoutBase,
} from "./tema";
...
export default function TandemRepeatsPlot({ sras, especies, altura }) {
  ...
  const figura = useMemo(() => {
    ...
    individuos.forEach((chave, linha) => {
      ...
      formas.push({
        ...
        fillcolor: TRILHA, line: { width: 0 }, layer: "below",
      });
      ...
      individuo.loci.forEach((locus) => {
        ...
        const infoLocus =
          `<b>${chave.replace(/_/g, " ")}</b> · ${locus.locus}` +
          `<br>${milhar(locus.inicio, language)}–${milhar(locus.fim, language)} ` +
          `(${locus.extensao_pb} ${t.pb})` +
          `<br>${locus.copias} ${t.copias} × ${locus.motivo_pb} ${t.pb}` +
          `<br>${t.zona} ${locus.zona} · ${locus.pct_da_cr}% ${t.daCr}`;

        linhasHoverX.push(locus.inicio, locus.fim, null);
        linhasHoverY.push(centro, centro, null);
        textosHover.push(infoLocus, infoLocus, null);
      });
    });
    ...
    return {
      data: [
        {
          type: "scatter", mode: "lines",
          x: linhasHoverX, y: linhasHoverY,
          line: { width: 22, color: "rgba(0,0,0,0)" },
          text: textosHover, hoverinfo: "text", showlegend: false,
        },
  ...
  }, [dados, individuos, modo, altura, t, language]);
  ...
  if (individuos.length === 0) {
    return (
      <div className="flex flex-col items-start gap-1">
        <p className="text-sm text-neutral-500">{t.semDados}</p>
        <p className="text-xs text-neutral-400">{t.porEspecie}</p>
      </div>
    );
  }
    ...
    <div>
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        <SeletorDeModo
          ...
          valor={modo}
          aoMudar={setModo}
          modos={["mapa", "dispersao"].map((chave) => ({ chave, texto: t[chave] }))}
        />
        {individuos.length === 1 && (
          <p className="text-xs" style={{ color: INK_MUTED }}>
            {t.resumo
              .replace("{n}", dados.individuos[individuos[0]].n_loci)
              .replace("{pb}", milhar(dados.individuos[individuos[0]].tr_total_pb, language))
      ...
      {ressalvas.length > 0 && (
        <div className="mt-3 flex items-start gap-2 rounded-md bg-amber-50 p-2.5 text-xs text-amber-900">
          ...
          <div className="flex flex-col items-start">
            <p className="font-bold">{t.ressalva}</p>
