      ...
      <HStack justify="space-between" align="center" mb={2} flexWrap="wrap" gap={2}>
        <ButtonGroup size="xs" isAttached variant="outline">
          {modos.map((chave) => (
            <Button
              ...
              bg={modoAtual === chave ? "#037373" : "white"}
              color={modoAtual === chave ? "white" : "#365B6D"}
              borderColor="#037373"
              _hover={{ bg: modoAtual === chave ? "#025f5f" : "#e6f2f2" }}
            >
              {t[chave]}
            </Button>
          ))}
        ...
        {amostras.length === 1 && (
          <Text fontSize="xs" color={INK_MUTED}>
            {dados.amostras[amostras[0]].total_codons.toLocaleString("pt-BR")}{" "}
            {t.total} · {t.semViés}
          </Text>
        )}
      </HStack>
